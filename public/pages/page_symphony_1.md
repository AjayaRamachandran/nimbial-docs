> This document is intended for early snapshots of Symphony v1.1 (and older). Newer installers offer a one-click experience.

# Get Symphony for Windows

## Step 1 - Download
Go to [symphony.nimbial.com](https://symphony.nimbial.com) and navigate to the **Download** tab.

Press **"Download Symphony for Windows"**

> If the platform shown on the button does not match the system you are on, scroll down to **Releases**, and select the latest version for your system.

A .zip file titled **Symphony-v1.1-win64-installer.zip** should begin downloading.

Once the file has finished downloading, unzip the file, preferably in a temporary folder like **Downloads**. Open it once it has been unzipped.

![Windows Unzipped Folder](/assets/windows-unzipped-folder.png)


## Step 2 - Install
Double-click **Symphony Setup 1.1.0.exe**. You should be prompted with a momentary setup wizard. If there are any instructions, follow them, then close the installer.

Symphony is now installed. You can find it as an app via **Search** or on your desktop.